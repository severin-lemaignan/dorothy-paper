...
\title{Generalized Social Dynamics in Human-Robot Interaction}
...
Relying on free play as a prototypical open task, we investigate the social
dynamics of children dyads playing together. By analyzing them from the
...
\begin{itemize}
    \item The robot who pretend-played with me
    \item I Guess Your Next Move! Leveraging Social Cues for Robots to
    ...
    \item A Robot Which Adapts its Social Behaviour to its Partner's Age
...
(\emph{situation awareness}), and second, to predict the participants' actions.
...
invited to freely interact with characters and items on an interactive table,
telling and acting stories with no explicit goal set in advance.
...
Protoimperative vs protodeclarative pointing~\cite{baron1989perceptual}
